...
import java.io.*;
import static com.blackmanatee.knitone.Sugar.*;
...
public class MainActivity extends Activity implements AdapterView.OnItemSelectedListener,OnValueChangeListener{
    ...
    public void onCreate(Bundle savedInstanceState){
		...
		ManaTB tb = new ManaTB();
		stCon = new Contract("stitch",new String[]{"name","base","result"},new int[]{Contract.T_TEXT,Contract.T_INT,Contract.T_INT},new int[]{0,0,0},new String[]{"Stitch Name","Base","Result"});
		tb.setDb(DB_NAME);
		...
		Spinner s = findViewById(R.id.select);
		s.setAdapter(pa);
		s.setOnItemSelectedListener(this);
		NumberPicker np = findViewById(R.id.rowPick);
